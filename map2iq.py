...
'''
def run_get_score(start_file,iq_file=None, r=None):
  global ip_path
  global rmax
  pid=os.getpid()
  if iq_file is None:
  ...
  if r is None:
    r=rmax
  voxel2pdb.write_pdb(start_file,'%s/temppdb/%d.pdb'%(output_folder,pid),rmax)
  args=['structure=%s/temppdb/%d.pdb'%(output_folder,pid),'experimental_data=%s'%iq_file]
  score=sas_I.run(args)
  return score
'''
...
class ED_map(object):
  ...
  '''
  def target(self, map):
    noise=np.loadtxt('/home/liucan/Desktop/hehao/experimental_data/iqfix_result/average_noise.txt')
    ...
    calculate_iq=normalize(self.calc_i)
    experimental_iq=normalize(self.data.i)
    
    if iq_noise.size<experimental_iq.size:
      iq_noise.resize((experimental_iq.size,))
    experimental_iq=experimental_iq+iq_noise[:experimental_iq.size]
    score = np.linalg.norm((calculate_iq-experimental_iq))
  ...
  def target(self, map):
    ...
    distance=(self.calc_i-self.data.i)
    #distance=(self.calc_i-self.data.i)/self.data.i
    distance=np.array(distance)

    '''  
    weight=np.zeros(shape=len(distance),dtype=float)
    baseweight=np.array([1.0,0.9,0.8,0.7,0.6])
    ...
    #weight=np.linspace(1,0.5,len(distance))
